"""FastAPI application for llm-ops-platform."""
...
app = FastAPI(title="llm-ops-platform", version="0.1.0", description="llm-ops-platform API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "llm-ops-platform", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "llm-ops-platform", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "llm-ops-platform", "version": "0.1.0", "ready": True,
